...
class TestBuild(unittest.TestCase):
    ...
    def test_4in6(self):
        ...
        writer.insert_network("1.1.0.0/24", record1)
        writer.insert_network("fe80::/16", record2)
        writer.to_file(self.filename)
        for mode in (maxminddb.MODE_MMAP_EXT, maxminddb.MODE_MMAP, maxminddb.MODE_FILE):
            m = maxminddb.open_database(self.filename, mode=mode)
    ...
    def test_insert_subnet(self):
        writer = MMDBWriter()
        writer.insert_network("1.0.0.0/8", record1)
        writer.insert_network("1.10.10.0/24", record2)
        writer.to_file(self.filename)
        for mode in (maxminddb.MODE_MMAP_EXT, maxminddb.MODE_MMAP, maxminddb.MODE_FILE):
            m = maxminddb.open_database(self.filename, mode=mode)
            self.assertEqual(record1, m.get("1.1.0.1"), mode)
    ...
    def test_int_type(self):
        ...
        for int_type, value_range in value_range_map.items():
            ...
            writer.insert_network("1.0.0.0/8", {"value": ok_value})
            writer.to_file(self.filename)
            for bad_value in (bad_value1, bad_value2):
                with self.assertRaises((ValueError, struct.error)):
                    writer.insert_network("1.0.0.0/8", {"value": bad_value})
                    writer.to_file(self.filename)
